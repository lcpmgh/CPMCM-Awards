...
#     v3.1：2025-12-29  小修：增加了对培养单位标准化文件、培养单位信息文件的重复值检查函数和步骤，添加了培养单位查询函数和步骤，修改了一些细节
...
my_read_csv  <- function(file_name, raw_data = F, slience = T, ...){
  #判断是否在读取获奖名单原始数据
  if(isTRUE(raw_data)){
  ...
  if(!slience) message(sprintf("已读取UTF-8文件【%s】", file_name))
  return(data)
}                 #创建读取函数（强制UTF-8）
...
data_collection <- function(international=F){
  ...
  for(file_name in files_name){
    t <- my_read_csv(file_name, raw_data = T, slience = F, integer64 = 'character', select = 1:9)   #转换队号列数据类型，排除2020年后的备注列
  ...
  data_total[, T_id:=1:nrow(data_total)]                                                  #生成唯一的队伍id，注意在数据准备好后再生成
...
# 2.7 对比数据中单位与单位库，用于查看数据中单位是否有新加入的（不再考虑爬虫得到的数据，相关函数见旧版）
...
# 2.8 检查手工修改后的0-unit_name_convert.csv和3-unit_info_dataset.csv是否有单位重复的问题。
unit_duplicated_check <- function(){
  # 检查步骤，
  # 1查看修改后的0-unit_name_convert.csv中：1.original是否有重复，2.original和standard是否有重复。
  # 2查看修改后的3-unit_info_dataset.csv中：unit是否有重复。
  # 参数定义
  test_name_convert <- my_read_csv("0-unit_name_convert.csv")
  test_name_info <- my_read_csv("3-unit_info_dataset.csv")
  or_name <- test_name_convert$original
  st_name <- test_name_convert$standard
  info_name <- test_name_info$Unit
  or_leng <- length(or_name)
  
  # 查找重复
  dp_or_name <- duplicated(or_name)
  dp_or_st_name <- rep(F, length=or_leng)
  for(i in 1:or_leng){
    dp_or_st_name[i] <- or_name[i] %in% st_name
  }
  dp_info_name <- duplicated(info_name)
  
  # 判断0-unit_name_convert.csv是否有问题
  if(sum(dp_or_name)>0){
    message("0-unit_name_convert.csv中，原始单位名存在重复，请检查以下问题：")
    message(paste0(or_name[dp_or_name], collapse=", "))
  }
  
  if(sum(dp_or_st_name)>0){
    message("0-unit_name_convert.csv中，原始单位名与标准单位名存在重复，请检查以下问题：")
    message(paste0(or_name[dp_or_st_name], collapse=", "))
  }
  
  if(sum(dp_or_name)+sum(dp_or_st_name)==0){
    message("经检查，0-unit_name_convert.csv文件没有问题！")
  }
  
  # 判断3-unit_info_dataset.csv是否有问题
  if(sum(dp_info_name)>0){
    message("3-unit_info_dataset.csv中，单位名存在重复，请检查以下问题：")
    message(paste0(info_name[dp_info_name], collapse=", "))
  } else{
    message("经检查，3-unit_info_dataset.csv文件没有问题！")
  }
}

# 2.9 区分个体并计算成员连续获奖次数，v7.0修改后，此函数只能用于识别Series>1的情形
...
# 2.11 其他函数，为了数据和程序维护
# 1.根据单位名，查询参赛记录和归一化规则，尤其是标准化之前的记录
unit_find <- function(data_total_original, unit){
  name_convert <- my_read_csv("0-unit_name_convert.csv")
  f_unit_info <- to_data_unit(data_total_original) %>% .[Unit==unit,]
  f_team_id <- unique(f_unit_info$T_id)
  f_team_info <- data_total_original[T_id %in% f_team_id, ] %>% setDF()
  f_name_convert <- name_convert[original==unit,] %>% setDF()
  if(nrow(f_team_info)==0) f_team_info <- "未查到参赛记录"
  if(nrow(f_name_convert)==0) f_name_convert <- "未查到归一化规则"
  res <- list("所查询的单位" = unit, 
              "该单位的参赛记录" = f_team_info,
              "该单位的归一化规则"=f_name_convert)
  return(res)
}
 
##### 3. 数据处理-生成总数据、数据单位名矫正、生成成员数据 #####################
# 3.1 将年份获奖数据整合，不考虑国际赛道
data_total_original <- data_collection(international=F)                                     

# 3.2 培养单位名称归一化
data_total <- unit_standardization(data_total_original)               

# 3.3 生成培养单位信息（初步，后续需人工查验补齐）
# 网络爬虫生成的本地数据库不再更新，v7.0后这部分内容已删掉，要想查看见上一个版本
# 手动查漏补缺步骤：
# 首先运行此函数，查看输出结果中，若存在NA值，则可能有以下两种原因：
#    1.新增培养单位
#    2.旧培养单位新增不规范的名字
# 对于1，需要修改3-unit_info_dataset.csv文件，添加新培养单位的信息（此文件中培养单位都是标准名称）
# 对于2，需要修改0-unit_name_convert.csv文件，添加新的培养单位标准化规则
# 至于是1还是2，需要将unit_info_dataset保存为999-temp-unit_info_dataset.csv挨个查看
# 如果某个country或province为NA的unit，向下找，找不到同单位的，说明是1，找到说明是2
# 注意，在3-unit_info_dataset.csv中，不要添加重复名字
# 结束这两步后，重新运行3.1和3.2，直至此函数运行结果中无NA值为止，crawler=F
...
# 3.4 查看经过手工修改后的0-unit_name_convert.csv和3-unit_info_dataset.csv是否有单位重复的问题
unit_duplicated_check()

# 3.5 生成获奖名单（处理后的）中的单位名称集（唯一化）（用于培养单位名称标准化手动的查漏补缺）
...
# 3.7 其他
# 1. 查看某个单位的参赛记录，尤其是标准化之前的
unit <- "中国科学院软件研究所"
uf_data <- unit_find(data_total_original, unit);uf_data   #归一化之前
uf_data <- unit_find(data_total, unit);uf_data            #归一化之后
