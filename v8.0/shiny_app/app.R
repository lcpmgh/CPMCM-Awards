...
ui <- fluidPage(
  ...
  tags$head(tags$title("CPMCM-Awards")),
  ...
  tabsetPanel(id = "mainPanel",
              ...
              tabPanel(title = "历年赛题",
                       div(class = "flex-container",
                           wellPanel(id = "pageL5", uiOutput("ui_p5_sidebar")),
                           wellPanel(id = "pageR5", uiOutput("ui_p5_main"))
                       ))
  ),
  tags$hr(),
...
server <- function(input, output, session){
  ...
  questions <- my_read_csv("0-questions_archive.csv")
  ...
  output$ui_p5_main <- renderUI({
    tagList(
      h3("历年赛题"),
      dataTableOutput("p5_atable")
    )
  })
  
  # p5表，获奖名单 ================================================================ 
  output$p5_atable <- renderDT({
    if(length(input$p5_input_Syearsession) == 0){
      sYear <- unique(questions$Year)
    } else {
