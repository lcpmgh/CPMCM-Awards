...
4. 本文件夹中文件的来源、内容和用途：
   ...
   + **0-questions_archive.csv**：历年赛题，每年更新；
   + **0-typeface.ttc**：微软雅黑字体，用于可视化程序中，指定字体，现已弃用；
   + **0-unit_info_dataset-backup.csv**：3-unit_info_dataset.csv的备份，不定时更新。data_processing.r中，第4.2部分很容易误运行，从而覆盖其中额外的培养单位信息，使其只包含data_total中的信息；
   + **0-unit_location_convert.csv**：保存省级行政区长名、短名和外国国家名，本是用于网络爬虫获取培养单位信息后，将名字转换为长名，在v4.0后，由于改用REmap，此步骤已不需要；
